...
class TwitterError(IOError): pass

def request_oauth_resource(consumer, url, access_token, parameters=None, 
        signature_method=signature_method, http_method='GET'):
    """
...
def post_tweet(consumer, connection, access_token, message):
    try:
...
def send_dm(consumer, connection, access_token, to_username, message):
    try:
        oauth_request = request_oauth_resource(consumer, 
                'https://twitter.com/direct_messages/new.json', 
                access_token, { 
                    'user': to_username,
                    'text': message
                }, http_method='POST')
...
def follow(consumer, connection, access_token, username):
    try:
        oauth_request = request_oauth_resource(consumer, 
