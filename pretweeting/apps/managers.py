...
class BulkInsert(models.Manager):
    ...
    def bulk_insert_commit(self, now=None, autoclobber=False, depth=0, max_depth=5, send_post_save=True, _self_ref=False, recover_pks=True, **kwargs):
        """
        ...
        try:
            ...
            # option to not do this added by gabe
            # use sparingly
            if recover_pks:
            ...
            if not _self_ref:
                if many_to_many:
                    self._many_to_many_insert(depth, max_depth,
                            autoclobber)
                if many_to_one:
                    self._many_to_one_insert(depth, max_depth,
                            autoclobber)
